...
public class ObjectsModel<T> {
	...
	MySQLHelper db = new MySQLHelper("//localhost/SchoolSystem", "root", "123");
	Class callerClass;
	...
	public T conertToObject(List<String> row) {
		try {
			...
			for (int i = 0; i < fields.length; i++) {
				...
				case "int":
					setter.invoke(instance, Integer.parseInt(row.get(i)));
					break;
				case "String":
					setter.invoke(instance, row.get(i));
					break;
				}
			}
	...
	public T getById(int id) {
		...
		return conertToObject(db.selectFrom(tableName, "id = " + id).get(0));
